angular.module('problems', ['ngMaterial', 'ngSanitize'])
	...
	$scope.gen_prob = function () {
	    $scope.prob = {};
	    $scope.prob.bi1 = [get_term(false), get_term()];
	    $scope.prob.bi2 = [get_term(false), get_term()];
	    $scope.prob.ops = [ops[rand_num(0, 1)], ops[rand_num(0, 1)]];

	    $scope.problem = '(' + term_to_str($scope.prob.bi1[0]);
	    $scope.problem += ' ' + $scope.prob.ops[0] + ' ';
	    $scope.problem += term_to_str($scope.prob.bi1[1]) + ')';
	    $scope.problem += ' (' + term_to_str($scope.prob.bi2[0]);
	    $scope.problem += ' ' + $scope.prob.ops[1] + ' ';
	    $scope.problem += term_to_str($scope.prob.bi2[1]) + ')';

	    var prob_str = '(' + term_to_str($scope.prob.bi1[0], true) + $scope.prob.ops[0]
		+ term_to_str($scope.prob.bi1[1], true) + ') * ('
		+ term_to_str($scope.prob.bi2[0], true) + $scope.prob.ops[1]
		+ term_to_str($scope.prob.bi2[1], true) + ')';
